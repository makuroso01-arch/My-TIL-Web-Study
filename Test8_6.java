...
//        }
    //효율적 풀이
        //main 메서드 -> 질문 던지는 곳
        for (int i = 2; i <= 30; i++) {
          if (isPrime(i)) {// 여기서 전문가를 불러서 "결과값"을 i(편지)로 받아옴
            System.out.print(i + " "); // 결과가 true(참)일 때만 실행
          }
        }
      }
    ...
    public static boolean isPrime(int n) {// 이 메서드는 대답(boolean)을 돌려줄 거야! *isPrime = 소수입니까?
      if (n < 2) return false; // 2보다 작으면? "아니, 소수 아니야(false)"
      // i * i <= n 은 i <= Math.sqrt(n)과 같습니다. *Math.sqrt(n)는 제곱근 함수.
        // 컴퓨터는 단순한걸 좋아함!! 제곱근 연산 보다는 단순 곱셈 연산을 선호함.
      for (int i = 2; i * i <= n; i++) {
        if (n % i == 0) return false; // 나누어떨어지는 걸 찾으면? "응, 아니야(false)" 하고 바로 퇴근
      }
     //위의 모든 검사를 통과했다면?
